...
void aimbot::run()
{
	...
	if ( !prediction::can_fire() && !revolver_pass )
	{
		...
		if ( !prediction::can_fire() )
		{
			...
			// Between-shots movement: when the weapon is cocking/cycling the next
			// bullet and we are far enough from the next fire time, release the
			// stop so the player can full-move (especially on snipers). We
			// re-acquire the stop in the final ticks before the next shot so the
			// follow-up is accurate. This keeps "Between shots" active (the stop
			// still kicks in to prepare the next shot) but removes the sluggish
			// slow-walk that used to last the entire cocking window.
			if ( config::current_config( weapon )->autostop_inbetween->get<bool>() && aimbot_helpers::between_shots_allow_movement() )
				keep_stop = stop_type::stop_none;
...
void aimbot::get_best_target( final_target_t& final_target )
{
	...
	static auto calc_target = [config] ( target_data_t* target ) -> std::vector<final_target_t>
	{
		...
		// Scan the full record set returned by get_scan_records, in priority
		// order. get_scan_records already appends in priority order: latest,
		// shot, oldest, distant, angle_diff, defensive, uncrouched. The old
		// code threw away everything except latest + oldest, which meant shot
		// records (the most important for resolver feedback) and defensive /
		// angle_diff records were never scanned — a major cause of dumping
		// magazines into desyncing enemies. We now scan them all, with an
		// early-exit when we find a lethal safepoint so the extra records
		// only cost time when the priority records didn't yield a kill.
		const auto player = target->player;

		// Fallback head-force-shot candidate. If we find a head hitpoint that
		// would deal lethal (or >=30) damage but fails the strict safety
		// checks (safety_no_roll / safety_full), we remember it. If the whole
		// scan yields no safe target and we've missed < 2 shots on this player,
		// we emit the head point anyway — better to shoot an exposed head than
		// to never fire. Gated on the miss count so a bad resolver read doesn't
		// cause repeated forced misses.
		final_target_t head_force{};
		bool has_head_force = false;
		const auto head_force_eligible = [ & ] () -> bool
		{
			const auto& log = player_log::get_log( target->index );
			return !player->get_player_info().fakeplayer && log.m_shots < 2;
		};

		for ( auto it = scan_records.begin(); it != scan_records.end(); ++it )
		{
			auto* record = *it;
			if ( !record || record->m_dormant || record->m_ignore )
				continue;
			...
			out.emplace_back( player, record, point );

			const auto health = player->get_predicted_health();
			if ( point.safety >= penetration::safety_no_roll && point.damage >= health )
				break;

			// Track a non-safe head point as a force-shot candidate. We only
			// consider head hitboxes with enough damage to be worth the gamble.
			if ( !has_head_force && head_force_eligible() && point.hitbox == HITBOX_HEAD && point.damage >= 30 && point.damage >= health )
			{
				head_force = final_target_t( player, record, point );
				has_head_force = true;
			}
		}

		// No safe target found, but we have an exposed head — force the shot
		// rather than stalling. This is the main fix for the "not shooting
		// exposed heads" symptom: strict safety filters used to reject the
		// point entirely, leaving the aimbot unable to fire.
		if ( out.empty() && has_head_force )
			out.emplace_back( head_force );
		...
		{
			// Adaptive early-stop extrapolation. The lookahead distance scales with
			// our unpredicted velocity so fast peeks trigger the stop earlier. The
			// base 4-tick lookahead is kept as a floor; faster movement adds up to
			// a few extra ticks of look-ahead so the stop catches the peek in time.
			const auto speed = prediction::unpred_vel.Length2D();
			const auto lookahead_ticks = 4.f + std::min( 6.f, speed / 90.f );
			aimpoint_t point = {};
			if ( !hitscan::get_best_hitpoint( target, *scan_records.front(), point, nullptr, local_player->get_eye_pos() + prediction::unpred_vel * interfaces::globals()->interval_per_tick * lookahead_ticks ) )
				return out;
...
//#endif
			...
			if ( config->predictive_autostop->get<bool>() && final_targets.empty() && !vars::aim.fake_duck->get<bool>() && prediction::unpred_vel.Length2D() > 20.f )
			{
				...
				const auto lookahead_ticks = 4.f + std::min( 6.f, speed / 90.f );
				point = {};
				if ( hitscan::get_best_hitpoint( target, log.extrapolated_record, point, extra, local_player->get_eye_pos() + prediction::unpred_vel * interfaces::globals()->interval_per_tick * lookahead_ticks ) )
...
//#if defined(ALPHA) || !defined(RELEASE)
	//visuals::players[ target.player->EntIndex() ].debug.hc = target.point.hitchance;
//#endif
	// Delay-shot handling with an adaptive timeout. The original code
	// returned unconditionally whenever the target was breaking lag
	// compensation and the shot had to be delayed, which trapped the aimbot
	// in an autostop loop against tickbase-shifting / defensive-doubletap
	// enemies and never fired even when the head was fully visible. We now
	// allow the delay for at most 3 ticks; after that we fall through and
	// fire at the best available (extrapolated) record so we don't get
	// stuck. The counter is reset whenever we actually fire or the target
	// stops breaking LC.
	auto& target_log = player_log::get_log( target.player->EntIndex() );
	if ( target.record->breaking_lc() && target.record->can_delay_shot() && target.record->delay_shot() )
	{
		target_log.m_delay_ticks++;
		if ( target_log.m_delay_ticks <= 3 )
		{
//#if defined(ALPHA) || !defined(RELEASE)
...
//#endif
		...
		// Timeout expired — fall through and fire at the current best record.
	}
	else
	{
		target_log.m_delay_ticks = 0;
	}
...
//#endif
	...
	// We fired — clear the per-target delay-shot timeout so the next engagement
	// starts fresh.
	player_log::get_log( target.player->EntIndex() ).m_delay_ticks = 0;
