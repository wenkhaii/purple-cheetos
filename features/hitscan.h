...
namespace hitscan
{
	// 3-matrix safepoint check data structure
	...
	void calculate_hitbox_data( hitbox_data_t& out, C_CSPlayer* player, int hitbox, matrix3x4_t* matrix );
	bool is_safe_point( const Vector& eye_pos, const Vector& aim_point, C_CSPlayer* player, lag_record_t& record, lag_record_t* extra = nullptr );
	...
	void setup_skeet_hitbox_points( circular_buffer<aimbot::aimpoint_t, 78>& points, int index, const studiohdr_t* studio_hdr, lag_record_t& record, const Vector& eyepos, float spread_radius, int safety, CCSWeaponData* weapon_data, const weaponconfig_t* config, int minimum_damage, bool multipoint_enabled );
