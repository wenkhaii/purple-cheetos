...
void visuals::player_info( const visual_data_t& player, layer& layer )
{
	...
	const auto info_right = [&] ()
	{
		...
		if ( vars::visuals.esp.flags.money->get<bool>() )
		{
			...
			layer.add_text( draw.get_font( FNV1A( "esp" ) ), vec2( player.top.x + player.width + 4, player.top.y - 3 + 8 * elements++ + 2 ), util::format( "$%d", player.money ), color, layer::align_left );
		}
		...
		{
			//const auto color = player.dormant ? ::color::gray( 128, player.alpha / 255.f ) : ::color( 255.f, 255.f, 255.f, player.alpha, true );
			layer.add_text( draw.get_font( FNV1A( "esp" ) ), vec2( player.top.x + player.width + 4, player.top.y - 3 + 8 * elements++ + 2 ), "HIT", ::color(204, 201, 201, 255 * player.alpha, true), layer::align_left );
		}
		if ( player.scoped && vars::visuals.esp.flags.scoped->get<bool>() )
		{
			...
			layer.add_text(draw.get_font(FNV1A("esp")), vec2(player.top.x + player.width + 4, player.top.y - 3 + 8 * elements++ + 2), "ZOOM", ::color(120, 160, 200, 255 * player.alpha, true), layer::align_left);
		}
		...
		//	const auto color = player.dormant ? ::color::gray( 128, player.alpha / 255.f ) : ::color( 210.f, 0.f, 0.f, player.alpha, true );
			layer.add_text( draw.get_font( FNV1A( "esp" ) ), vec2( player.top.x + player.width + 4, player.top.y - 3 + 8 * elements++ + 2 ), "FD", ::color(215, 255, 255, 255 * player.alpha, true), layer::align_left );
		}
		if ( player.lc && !player.dormant && vars::visuals.esp.flags.lc->get<bool>() )
		{
			//const auto color = player.dormant ? ::color::gray( 128, player.alpha / 255.f ) : ::color( 130.f, 180.f, 0.f, player.alpha, true );
			layer.add_text( draw.get_font( FNV1A( "esp" ) ), vec2( player.top.x + player.width + 4, player.top.y - 3 + 8 * elements++ + 2 ), "LC", ::color(230, 80, 80, 255 * player.alpha, true), layer::align_left );
		}
		if ( player.xp && !player.dormant && vars::visuals.esp.flags.xp->get<bool>() )
		{
			...
			layer.add_text( draw.get_font( FNV1A( "esp" ) ), vec2( player.top.x + player.width + 4, player.top.y - 3 + 8 * elements++ + 2 ), "XP", color, layer::align_left );
		}
...
/*	layer.add_rect_filled(rect_legacy({player.top.x - player.width - 5 - elements * 6, player.top.y}, {4, player.height}), ::color(0, 0, 0, static_cast<int>(player.alpha * 0.4f)));
	...
	if (!vars::visuals.esp.health_style->get<int>()) {
		...
		/*layer.add_rect_filled(rect_legacy({player.top.x - player.width - 4 - elements * 6, player.top.y + player.height - height - 1}, {2, height}), ::color(16, 16, 16, static_cast<int>(220 * player.alpha * 0.4f)));
		layer.add_rect_filled(rect_legacy({ player.top.x - player.width - 3 - elements * 6, player.top.y + player.height - height - 1 }, { 1, height - 2 }), ::color(10, 10, 10, static_cast<int>(245 * player.alpha * 0.4f)));
		layer.add_rect_filled(rect_legacy({ player.top.x - player.width - 4 - elements * 6, player.top.y + player.height - height + 1 }, { 2, 1 }), ::color(10, 10, 10, static_cast<int>(245 * player.alpha * 0.4f)));
		layer.add_rect_filled(rect_legacy({ player.top.x - player.width - 4 - elements * 6, player.top.y + player.height - 1 }, { 2, 1 }), ::color(10, 10, 10, static_cast<int>(245 * player.alpha * 0.4f)));
		layer.add_rect_filled(rect_legacy({ player.top.x - player.width - 3 - elements * 6, player.top.y + player.height - height + 1 }, { 1, height - 2 }), color);*3/
	}
	else {
		layer.add_rect_filled_multicolor(rect_legacy({ player.top.x - player.width - 4 - elements * 6, player.top.y + player.height - height - 1 }, { 2, height }), {
			::color(16, 16, 16, static_cast<int>(220 * player.alpha * 0.4f)),
			::color(10, 10, 10, static_cast<int>(245 * player.alpha * 0.4f)),
			::color(10, 10, 10, static_cast<int>(245 * player.alpha * 0.4f)),
			color
			});
	}*/
	//	float health_fraction = std::clamp(1 - health / 100.f, 0.f, 1.f);
//layer.add_rect_filled(rect_legacy({ player.top.x - player.width - 3 - elements * 6, player.top.y + 1 + height * health_fraction }, { 1, height - 2 }), color);
	//layer.add_rect_filled(rect_legacy({ player.top.x - player.width - 3 - elements * 6, player.top.y + 1 + height * health_fraction }, { 1, height - 2 }), color);
	...
	if ( health < 99.f && health >= 1.f )
		layer.add_text( draw.get_font( FNV1A( "esp" ) ), vec2( player.top.x - player.width - 3, player.top.y + player.height - height ), util::format( "%d", player.health ), ::color( 255, 255, 255, static_cast< int >( player.alpha ) ), layer::align_center, layer::align_center );
}
		...
		else if (id == ClassId::CBaseCSGrenadeProjectile)
		{
			...
			if (model)
			{
				...
				if (hdr)
				{
					...
					if (strstr(hdr_name, "flashbang"))
					{
						data.name = "Flashbang";
					}
					else if (strstr(hdr_name, "fraggrenade"))
					{
						data.name = "HE Grenade";
					}
...
void visuals::aa_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), text, color( Color(255, 255, 255, 255).direct() ), layer::align_left, layer::align_center );
...
void visuals::fakeduck_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	{
		layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "NO FD" , color( Color::RedAccent().direct() ), layer::align_left, layer::align_center );
		offset += 24;
		return;
	}
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "FD", color( Color(255, 0, 50).fade_to( Color(255, 255, 255, 255), multiplier ).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::lean_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "LEAN", color( Color(255, 0, 50).fade_to( Color::Lime(), multiplier ).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::doubletap_indicator( const  misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "DT", color( Color(255, 0, 50).fade_to( Color(255, 255, 255, 255), multiplier ).direct() ), layer::align_left, layer::align_center );
	//x = 53 + len of text
	//layer.add_rect_filled_multicolor( rect_legacy( { vec2( 10, size.y / 1.65f + offset - 20 )}, { 53 + 27, 33 } ), { ::color( 0, 0, 0, 10 ), ::color( 0, 0, 0, 20 ), ::color( 0, 0, 0, 30 ), ::color( 0, 0, 0, 15 ) } );
...
void visuals::freestand_indicator( const  misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "FS", color( Color(255, 255, 255, 255).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::recharge_bar( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_rect_filled( rect_legacy( { 11, size.y / 1.65f + offset - 13 }, { width - 1, 2 } ), ::color( Color::RedAccent().fade_to( Color(255, 255, 255, 255), multiplier ).direct() ) );
...
void visuals::silent_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "HS", color( Color(255, 0, 50).fade_to( Color(255, 255, 255, 255), multiplier ).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::anti_exploit_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "AX", color( Color(255, 255, 255, 255).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::override_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "MD", color( Color(255, 255, 255, 255).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::force_safety_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "SAFE", color( Color(255, 255, 255, 255).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::hsonly_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	dx_adapter::icon( layer, vec2( 30, size.y / 1.65f + offset + 3 ), 36, color( Color::Black().direct() ), xorstr_( "hud\\deathnotice\\icon_headshot" ), layer::align_left, layer::align_center, false );
	dx_adapter::icon( layer, vec2( 30, size.y / 1.65f + offset + 2 ), 36, color( Color(255, 255, 255, 255).direct() ), xorstr_( "hud\\deathnotice\\icon_headshot" ), layer::align_left, layer::align_center, false );
	//_( fd_s, "HSO" );
	//layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), fd_s, color( Color(255, 255, 255, 255).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::force_baim_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "BAIM", color( Color::RedAccent().direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::lc_indicator( const misc_data_t& misc_data, int& offset )
{
	...
	layer.add_text( draw.get_font( FNV1A( "lby" ) ), vec2( 30, size.y / 1.65f + offset ), "LC", color( Color(123, 194, 21, 255).direct() ), layer::align_left, layer::align_center );
	offset += 24;
}
...
void visuals::autowall_crosshair(const misc_data_t& data)
{
	...
	const auto color = data.wallbang ? Color(123, 194, 21) : Color(255, 0, 50);
	...
	layer.add_rect_filled(rect_legacy({ (float)cross_x, (float)cross_y }, { 3, 3 }), ::color(0, 0, 0, 210));

	//vert
	layer.add_rect_filled(rect_legacy({ (float)cross_x + 1, (float)cross_y }, { 1, 3 }), ::color(color.direct()));
	layer.add_rect_filled(rect_legacy({ (float)cross_x, (float)cross_y + 1 }, { 3, 1 }), ::color(color.direct()));

	//layer.add_rect_filled( rect_legacy( { ( float ) cross_x + 1, ( float ) cross_y }, { 3, 3 } ), ::color( color.direct() ) );
}
