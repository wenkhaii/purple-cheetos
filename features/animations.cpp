...
void animations::calculate_sim_ticks( lag_record_t* record, C_CSPlayer* player, lag_record_t* previous )
{
	...
	// Integrate at the average playback rate across the lag window instead of
	// stepping from start rate to end rate at cycle wrap. The step-function
	// swap introduces error when the rate changed mid-lag (player accelerated
	// or decelerated between the two records). With only the two endpoint
	// rates available, the average is the best linear estimate of the
	// mid-lag rate. See cstrike15_src UpdateMovementActions for the server's
	// per-tick layer 11 cycle integration.
	const auto avg_playback_rate = ( start.m_flPlaybackRate + end.m_flPlaybackRate ) * 0.5f;

	record->m_ignore = true;

	for ( auto i = 1; i <= 64; i++ )
	{
		const auto cycle_delta = avg_playback_rate * ticks_to_time( 1 );
		cycle += cycle_delta;
	...
	if ( record->m_ignore )
	{
		// Standing transition: layer 6 weight is zero and playback is
		// negligible while on ground. This is a legitimate standing move
		// pattern, not a tickbase shift. Flagging it as m_tickbase_shift
		// causes the resolver to over-aggressively treat the record as
		// defensive and discard the eye yaw, which wastes shots on targets
		// that simply stopped walking.
		...
		record->m_lagamt = 1;
		record->m_tickbase_shift = !standing;
	}
...
void animations::update_player_animations( lag_record_t* record, C_CSPlayer* player, lag_record_t* previous )
{
	...
	auto fake = !bot && player->is_enemy() && local_player && local_player->get_alive() && !vars::legit_enabled() && vars::aim.resolver->get<bool>();
