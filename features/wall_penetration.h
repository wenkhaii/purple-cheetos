...
namespace penetration
{
	...
	// Multi-direction safepoint: verify a hitpoint deals >= min_damage
	// against ALL four resolver directions (networked, max, zero, min).
	// Returns true only if every direction yields damage >= min_damage.
	// This catches the case where the current resolved direction is a
	// hit but a plausible alternative direction would miss — the point
	// is not "safe" against resolver uncertainty.
	bool is_multidirection_safe( C_CSPlayer* shooter, const Vector& shotpos, const Vector& point, lag_record_t* record, CCSWeaponData* weapon_data, int min_damage = 1 );
