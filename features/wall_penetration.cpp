...
bool penetration::is_multidirection_safe( C_CSPlayer* shooter, const Vector& shotpos, const Vector& point, lag_record_t* record, CCSWeaponData* weapon_data, int min_damage )
{
	if ( !shooter || !record || !weapon_data )
		return false;

	// Verify the point deals >= min_damage against ALL four resolver
	// directions. If any direction misses, the point is not safe against
	// resolver uncertainty and should not be treated as a safepoint.
	const std::array< resolver_direction, 4 > directions = {
		resolver_direction::resolver_networked,
	...
	{
		aimbot::aimpoint_t test_point{};
		test_point.point = point;
		test_point.hitbox = -2;

		auto damage = 0;
		pen_data pd( record, dir, safety_none, nullptr, weapon_data );

		if ( !can_hit( shooter, pd, shotpos, &test_point, damage ) || damage < min_damage )
			return false;
	}
