...
bool lagcomp::run_extrapolation( const C_CSPlayer* player, const bool simple )
{
	...
	// Cap extrapolation to 14 ticks. The server rejects extrapolation
	// beyond sv_maxusrcmdprocessticks (default 16), and extrapolating
	// further than 14 ticks produces unreliable positions (velocity
	// prediction degrades over long horizons). This ensures we never
	// extrapolate beyond what the server will accept or what produces
	// useful hitboxes.
	log.extrapolated_record.m_lagamt = std::min( log.extrapolated_record.m_lagamt, 14 );
...
bool lagcomp::is_breaking_lagcomp( const C_CSPlayer* player, const lag_record_t* current )
{
	...
	// Simtime regression with large lag: the server sent an older simtime
	// (tickbase shift / choke exploit) AND the lag is > 10 ticks. This is a
	// direct breaking signal that the teleport-distance check below misses
	// for small-position breaks. The lagamt > 10 gate avoids false
	// positives from normal simtime jitter on short lags.
	const auto simtime_regressed = current->m_simtime <= previous.m_simtime && current->m_lagamt > 10;
	if ( simtime_regressed )
		return true;
