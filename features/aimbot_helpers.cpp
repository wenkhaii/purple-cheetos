...
bool aimbot_helpers::is_between_shots()
{
	const auto weapon = local_weapon;
	if ( !weapon || !local_player )
		return false;

	// Must be a shootable weapon that cycles between shots.
	if ( !weapon->is_shootable() && weapon->get_weapon_type() != WEAPONTYPE_KNIFE )
		return false;

	// If we're reloading, that's not "between shots" — reload is a separate state
	// the user can interrupt by switching weapons; we don't release the stop there.
	if ( weapon->in_reload() )
		return false;

	// If we can already fire, we're not between shots.
	if ( prediction::can_fire() )
		return false;

	// If the weapon has no pending next attack time, it's not cycling.
	const auto next_attack = weapon->get_next_primary_attack();
	const auto curtime = interfaces::globals()->curtime;
	if ( next_attack <= curtime )
		return false;

	// We can't fire and next_primary_attack is in the future → we are in the
	// cocking/cycling window between shots.
	return true;
}

bool aimbot_helpers::between_shots_allow_movement( int prepare_ticks )
{
	if ( !is_between_shots() )
		return false;
	...
	// Single-shot weapons (Scout, AWP, revolver) benefit most from free movement
	// during cocking — the cycle time is long and slow-walking feels terrible.
	// For these we always release the stop during the cocking window except in
	// the final ticks before the next shot.
	const auto is_singleshot = weapon->get_weapon_id() == WEAPON_SSG08
		|| weapon->get_weapon_id() == WEAPON_AWP
		|| weapon->get_weapon_id() == WEAPON_G3SG1
		|| weapon->get_weapon_id() == WEAPON_SCAR20
		|| weapon->get_weapon_id() == WEAPON_REVOLVER;

	// How many ticks before the next shot to re-acquire the stop. Single-shot
	// weapons need a bit more time to settle for an accurate follow-up; autos
	// can get away with less.
	const auto ticks_until_fire = time_to_ticks( weapon->get_next_primary_attack() - interfaces::globals()->curtime );

	if ( is_singleshot )
		return ticks_until_fire > prepare_ticks;

	// For non-single-shot weapons (rifles, pistols, autos) only release the stop
	// if we're clearly in the middle of the cycle. Near the end we re-acquire so
	// the next burst is accurate.
	return ticks_until_fire > std::max( prepare_ticks, 2 );
}
