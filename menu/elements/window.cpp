...
void window::run_render()
{
	...
	*/
	...
	/*
	* Base background — deep near-black with a subtle vertical gradient for depth.
	*/
	d.add_rect_filled_linear_gradient( rect_legacy( m_pos, m_size ), gui_color::custom( Color( 14, 14, 16 ) ), gui_color::custom( Color( 10, 10, 12 ) ) );
...
void window::header() const
{
	...
	*/
	if ( m_tabs.size() > 1 )
	{
		...
		d.add_rect_filled_linear_gradient( rect_legacy( { m_pos.x, m_pos.y + padding.y + 1 }, { m_size.x, size.y } ), gui_color::custom( Color( 28, 28, 32, 160 ) ), gui_color::custom( Color( 18, 18, 22, 140 ) ) );
