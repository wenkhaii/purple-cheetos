...
//#endif
	...
	// 3-matrix safepoint check utilities
	__forceinline void vector_itransform( const Vector& in, const matrix3x4_t& matrix, Vector& out )
	{
		Vector temp = in - Vector( matrix[ 0 ][ 3 ], matrix[ 1 ][ 3 ], matrix[ 2 ][ 3 ] );
		out.x = temp.Dot( matrix[ 0 ] );
		out.y = temp.Dot( matrix[ 1 ] );
		out.z = temp.Dot( matrix[ 2 ] );
	}

	__forceinline void vector_irotate( const Vector& in, const matrix3x4_t& matrix, Vector& out )
	{
		out.x = in.Dot( matrix[ 0 ] );
		out.y = in.Dot( matrix[ 1 ] );
		out.z = in.Dot( matrix[ 2 ] );
	}

	__forceinline bool intersect_bb( const Vector& start, const Vector& end, const Vector& mins, const Vector& maxs )
	{
		...
		for ( int i = 0; i < 3; i++ )
		{
			...
			{
				float inv_delta = 1.0f / delta[ i ];
				float t0 = ( mins[ i ] - start[ i ] ) * inv_delta;
				float t1 = ( maxs[ i ] - start[ i ] ) * inv_delta;
