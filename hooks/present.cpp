...
long __stdcall hook::present( IDirect3DDevice9* device, RECT* pSourceRect, RECT* pDestRect, HWND hDestWindowOverride, RGNDATA* pDirtyRegion )
{
	...
	if ( !adapter )
	{
		...
		draw.manage( FNV1A( "esp" ), std::make_shared<::gui::bitfont>( xorstr_( "Small Fonts" ), 8, 400, ff_outline || ff_antialias, 0, 0x80 ) );
		draw.manage( FNV1A( "esp_name" ), std::make_shared<font>( win_dir + xorstr_( "/fonts/segoeuib.ttf" ), 13.f, ff_shadow, 0, 0x45F ) );
		//draw.manage( FNV1A( "esp_name" ), std::make_shared<::gui::bitfont>( xorstr_( "Verdana" ), 12, 400, ff_outline, 0, 0x80 ) );
		//draw.manage( FNV1A( "esp_name" ), std::make_shared<bitfont>( xorstr_( "Verdana" ), 12.f, 400, 0, 0x45F ) );
		...
		//draw.manage( FNV1A( "lby" ), std::make_shared<font>( win_dir + xorstr_( "/fonts/verdanab.ttf" ), 26.f, ff_shadow, 0, 0x80 ) );
		draw.manage( FNV1A( "lby" ), std::make_shared<font>( win_dir + xorstr_( "/fonts/verdanab.ttf" ), 25, ff_shadow || ff_antialias, 0, 0x80 ) );
