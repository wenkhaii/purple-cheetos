...
void __stdcall cl_move(const float accumulated_extra_sample, const bool final_tick)
{
	...
	if (recharge)
	{
		...
		// Copy the last real command so the user's movement (forwardmove /
		// sidemove / viewangles) keeps flowing during recharge. The old code
		// copied the command but left IN_ATTACK/IN_ATTACK2 set, which caused
		// weapon stutter and the "jerk" feel during recharge. We strip the
		// attack buttons (matching fill_fake_commands) so recharge ticks can't
		// fire, and we keep tick_count = INT_MAX so the server rejects these
		// ticks for sim advancement while still accepting the movement for
		...
		interfaces::input()->m_pCommands[current_tick % 150].buttons &= ~(IN_ATTACK | IN_ATTACK2);
		...
		// Advance local prediction for the recharge tick so the player's view,
		// movement and animation continue smoothly instead of freezing for the
		// recharge duration (the "screen jerk" during DT recharge). We run the
		// prediction pass only — no network command is sent because we return
		// before original_cl_move. This keeps the camera glued to the player's
		// real movement while the server-side tickbase recharge happens out of
		// band.
		prediction::run_prediction( current_tick );
