...
void __fastcall hook::draw_3d_debug_overlays( void* ecx, void* edx )
{
	...
	if ( misc::has_position )
	{
		...
		chams::materials[ MAT_SPRITE ]->FindVar( "$color", nullptr )->set_vec_val( col.rBase() * 4, col.gBase() * 4, col.bBase() * 4 );
		...
		DrawSprite( misc::movement_position + Vector( 0, 0, 0 ), 80, 80, { 255, 255, 255, 255 } );
	}
	...
	if ( vars::aim.visualize_aimbot->get<bool>() )
		for ( const auto& target : aimbot::targets )
		{
			...
			for ( const auto& hitbox : target.hitboxes )
			{
				...
				chams::materials[ MAT_SPRITE ]->FindVar( "$color", nullptr )->set_vec_val( col.rBase() * 50.f * col.aBase(), col.gBase() * 50.f * col.aBase(), col.bBase() * 50.f * col.aBase() );
...
void __fastcall hook::shader_api_draw_mesh( void* ecx, void* edx, uintptr_t pMesh, int nCount, MeshInstanceData_t* pInstances, int nCompressionType, uintptr_t pCompiledState, uintptr_t pInfo )
{
	...
	else if ( material && vars::visuals.dark->get<bool>() && strstr( material->GetTextureGroupName(), "World" ) )
	{
		for ( auto i = 0; i < nCount; i++ )
		{
	...
	/*else if ( material && vars::visuals.dark->get<bool>() && strstr( material->GetName(), "vgui_white"  ) )
	{
		for ( auto i = 0; i < nCount; i++ )
		{
	...
	else if ( material && vars::visuals.skybox->get<int>() && strstr( material->GetTextureGroupName(), "SkyBox" ) )
	{
